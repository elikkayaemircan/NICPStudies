...
RandIn=$( ls "$MyPath"/data/genie/tungsten/"$NuFlav"/*/gntp.0.gst_below.root | shuf -n 1 )

echo "Checking for the output data directory.."
if [  ! -d "$MyPath"/data/geant/tungsten/"$NuFlav"/below/"$ClusterId"_"$ProcId" ]; then
  mkdir -p "$MyPath"/data/geant/tungsten/"$NuFlav"/below/"$ClusterId"_"$ProcId"
fi

cd /afs/cern.ch/user/e/eelikkay

echo "Processing $NuFlav flavor for geant events.."
alienv setenv FairShip/latest -c python /afs/cern.ch/user/e/eelikkay/sw/slc7_x86-64/FairShip/master-1/macro/run_simScript.py --Genie -f $RandIn -n $NuWght -o "$MyPath"/data/geant/tungsten/"$NuFlav"/below/"$ClusterId"_"$ProcId"
