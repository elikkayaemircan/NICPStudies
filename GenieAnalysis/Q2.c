{
    ...
    //Histogram setup
    TH1F *h1 = new TH1F("#nu_{e} + #bar{#nu}_{e}", "Momentum transfer Q^{2} of incoming #nu + #bar{#nu}", 50, 0, 80);
    h1->SetLineColor(2);
    h1->SetLineWidth(2);

    TH1F *h2 = new TH1F("#nu_{#mu} + #bar{#nu}_{#mu}", "Momentum transfer Q^{2} of incoming #nu + #bar{#nu}", 50, 0, 80);
    h2->SetLineColor(3);
    h2->SetLineWidth(2);

    TH1F *h3 = new TH1F("#nu_{#tau} + #bar{#nu}_{#tau}", "Momentum transfer Q^{2} of incoming #nu + #bar{#nu}", 50, 0, 80);
    h3->SetLineColor(4);
    h3->SetLineWidth(2);
    ...
    //Take the branch
    double Q2_e;
    auto Q2_e_brc = t_e->GetBranch("Q2");
    Q2_e_brc->SetAddress(&Q2_e);

    double Q2_e_bar;
    auto Q2_e_bar_brc = t_e_bar->GetBranch("Q2");
    Q2_e_bar_brc->SetAddress(&Q2_e_bar);

    double Q2_mu;
    auto Q2_mu_brc = t_mu->GetBranch("Q2");
    Q2_mu_brc->SetAddress(&Q2_mu);

    double Q2_mu_bar;
    auto Q2_mu_bar_brc = t_mu_bar->GetBranch("Q2");
    Q2_mu_bar_brc->SetAddress(&Q2_mu_bar);

    double Q2_tau;
    auto Q2_tau_brc = t_tau->GetBranch("Q2");
    Q2_tau_brc->SetAddress(&Q2_tau);

    double Q2_tau_bar;
    auto Q2_tau_bar_brc = t_tau_bar->GetBranch("Q2");
    Q2_tau_bar_brc->SetAddress(&Q2_tau_bar);
    ...
    h2->GetXaxis()->SetTitle("Q^{2} (GeV^{2})");
    h3->GetXaxis()->SetTitle("Q^{2} (GeV^{2})");
