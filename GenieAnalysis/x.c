{
    ...
    //Histogram setup
    TH1F *h1 = new TH1F("#nu_{e} + #bar{#nu}_{e}", "Bjorken x distribution for #nu + #bar{#nu}", 100, 0, 1);
    h1->SetLineColor(2);
    h1->SetLineWidth(2);

    TH1F *h2 = new TH1F("#nu_{#mu} + #bar{#nu}_{#mu}", "Bjorken x distribution for #nu + #bar{#nu}", 100, 0, 1);
    h2->SetLineColor(3);
    h2->SetLineWidth(2);

    TH1F *h3 = new TH1F("#nu_{#tau} + #bar{#nu}_{#tau}", "Bjorken x distribution for #nu + #bar{#nu}", 100, 0, 1);
    h3->SetLineColor(4);
    h3->SetLineWidth(2);
    ...
    //Take the branch
    double x_e;
    auto x_e_brc = t_e->GetBranch("x");
    x_e_brc->SetAddress(&x_e);

    double x_e_bar;
    auto x_e_bar_brc = t_e_bar->GetBranch("x");
    x_e_bar_brc->SetAddress(&x_e_bar);

    double x_mu;
    auto x_mu_brc = t_mu->GetBranch("x");
    x_mu_brc->SetAddress(&x_mu);

    double x_mu_bar;
    auto x_mu_bar_brc = t_mu_bar->GetBranch("x");
    x_mu_bar_brc->SetAddress(&x_mu_bar);

    double x_tau;
    auto x_tau_brc = t_tau->GetBranch("x");
    x_tau_brc->SetAddress(&x_tau);

    double x_tau_bar;
    auto x_tau_bar_brc = t_tau_bar->GetBranch("x");
    x_tau_bar_brc->SetAddress(&x_tau_bar);
    ...
    h2->GetXaxis()->SetTitle("Bjorken x");
    h3->GetXaxis()->SetTitle("Bjorken x");
